    ...
    public class Log
    {
        ...
        private void WriteBase(string logMessage, string filePath)
        {
            lock (_lSettings)
            {
                ...
                {
                    if ((outputs & Outputs.Custom) == Outputs.Custom)
                        _WriteToCustom = false;
                    if ((outputs & Outputs.Console) == Outputs.Console)
                        _WriteToConsole = false;
                    if ((outputs & Outputs.File) == Outputs.File)
                        _WriteToFile = false;
                }
        ...
        public static void CreateLogDirectory(string filePath)
        {
            ...
            var paths = filePath.Split('\\').Reverse().Skip(1).Reverse().ToArray();
